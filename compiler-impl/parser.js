/**
 * Mojo Compiler - Parser Phase (JavaScript)
 * 역할: 토큰 스트림 → AST 생성 (수정 버전)
 */
...
class Parser {
  ...
  parseStatement() {
    ...
    } else if (!this.match(TokenType.RBRACE, TokenType.EOF)) {
  ...
  parseReturnStatement() {
    ...
    let value = null;
    if (!this.match(TokenType.RBRACE, TokenType.SEMICOLON, TokenType.EOF)) {
      value = this.parseExpression();
    }
